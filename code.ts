...
function generateTableColor(index: number): { r: number; g: number; b: number } {
  const colors = [
...
figma.ui.onmessage = async (msg: { type: string; schema: string; showRelationships?: boolean }) => {
  if (msg.type === 'draw-schema') {
    try {
      ...
      await figma.loadFontAsync({ family: "Inter", style: "Regular" });      let x = 50, y = 50; // Start with some margin from edges
      const createdFrames = [];
      ...
      console.log(`Layout: ${maxTablesPerRow} tables per row, ${schema.tables.length} total tables`);      let currentRow = 0;
      let currentCol = 0;
      ...
      for (const rel of schema.relationships) {
        ...
        if (fromTable && toTable) {
          ...
          const distance = Math.sqrt(
            Math.pow(fromTable.frame.x - toTable.frame.x, 2) + 
      ...
      if (msg.showRelationships !== false) { // Default to true if not specified
        ...
        const tableColorMap = new Map<string, { r: number; g: number; b: number }>();
        ...
        for (const rel of schema.relationships) {
          ...
          if (fromTable && toTable) {
            ...
            const fromColumnIndex = fromTable.columns.findIndex((col: TableColumn) => col.name === rel.fromColumn);
            const toColumnIndex = toTable.columns.findIndex((col: TableColumn) => col.name === rel.toColumn);
